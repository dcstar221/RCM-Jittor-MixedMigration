...
from .custom_base_transformer_layer import MyCustomBaseTransformerLayer, JittorTransformerLayerSequence
import copy
import warnings
...
# Jittor-native RadarCameraGating (replaces PyTorch import for proper weight sync)
...
class RadarGuidedBEVEncoder(JittorTransformerLayerSequence):
    ...
    @staticmethod
    def get_reference_points(H, W, Z=8, num_points_in_pillar=4, dim='3d', bs=1, device='cuda', dtype=jt.float32):
        ...
        if dtype == torch.float16:
            ...
            dtype_jt = jt.float32
        elif dtype == jt.float16 or dtype == jt.float32:
            dtype_jt = dtype
        else:
            dtype_jt = jt.float32
        ...
        if dim == '3d':
            zs = jt.linspace(0.5, Z - 0.5, num_points_in_pillar).astype(dtype_jt).view(-1, 1, 1).expand(num_points_in_pillar, H, W) / Z
            xs = jt.linspace(0.5, W - 0.5, W).astype(dtype_jt).view(1, 1, W).expand(num_points_in_pillar, H, W) / W
            ys = jt.linspace(0.5, H - 0.5, H).astype(dtype_jt).view(1, H, 1).expand(num_points_in_pillar, H, W) / H
        ...
        # reference points on 2D bev plane, used in temporal self-attention (TSA).
            ...
            ref_y, ref_x = jt.meshgrid(
                jt.linspace(0.5, H - 0.5, H).astype(dtype_jt),
                jt.linspace(0.5, W - 0.5, W).astype(dtype_jt)
            )
    ...
    def point_sampling(self, reference_points, pc_range,  img_metas):
        ...
        lidar2img = reference_points.clone().assign(lidar2img)  # (B, N, 4, 4)
        ...
        reference_points_cam = jt.matmul(lidar2img.astype(jt.float32),
                                          reference_points.astype(jt.float32)).squeeze(-1)
        eps = 1e-5
        ...
        if hasattr(jt, 'nan_to_num'):
            bev_mask = jt.nan_to_num(bev_mask)
        else:
            bev_mask = bev_mask.clone().assign(
                np.nan_to_num(bev_mask.numpy()))
    ...
    def forward(self,
        ...
        output = bev_query
        intermediate = []
        torch_device = torch.device('cuda') if jt.flags.use_cuda else torch.device('cpu')
        torch_dtype = torch.float16 if bev_query.dtype == jt.float16 else torch.float32
        ref_3d = self.get_reference_points(
            bev_h, bev_w, self.pc_range[5]-self.pc_range[2], self.num_points_in_pillar, dim='3d', bs=bev_query.size(1), device=torch_device, dtype=torch_dtype)
        ref_2d = self.get_reference_points(
            bev_h, bev_w, dim='2d', bs=bev_query.size(1), device=torch_device, dtype=torch_dtype)
        ...
        **kwargs contains some specific arguments of attentions.
        ...
        for layer in self.operation_order:
            ...
            # spaital cross attention
                ...
                # gated attention fusion (pure Jittor, no cross-framework conversion)
