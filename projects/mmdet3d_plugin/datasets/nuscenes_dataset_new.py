...
class CustomNuScenesDatasetNew(NuScenesDataset):
    ...
    def __init__(self, json_file = None, queue_length=4, bev_size=(200, 200), overlap_test=False, data_root=None, test_mode=False, with_info =False, version='v1.0-trainval', max_samples=None, *args, **kwargs):
        super().__init__(data_root=data_root, test_mode=test_mode,*args, **kwargs)
        ...
        if max_samples is not None and len(self.data_infos) > max_samples:
            self.data_infos = self.data_infos[:max_samples]
            if hasattr(self, 'flag'):
                self.flag = self.flag[:max_samples]
            
        if self.with_info and not test_mode:
            print('nuscenes_dataset')
            self.data_info = NuScenes(version=self.version, dataroot=data_root, verbose=True)
    ...
    def get_data_info(self, index):
        ...
        def strip_prefix(path_str):
            if isinstance(path_str, str):
                path_str = path_str.replace('/Volumes/HP P500/', '')
                path_str = path_str.replace('data/v1.0-mini/', self.data_root)
                path_str = path_str.replace('data/nuscenes/', self.data_root)
                return path_str
            return path_str
        ...
        for s in info['sweeps']:
            s_cleaned = s.copy()
            s_cleaned['data_path'] = strip_prefix(s['data_path'])
            cleaned_sweeps.append(s_cleaned)
            
        input_dict = dict(
            sample_idx=info['token'],
            pts_filename=strip_prefix(info['lidar_path']),
            sweeps=cleaned_sweeps,
        ...
        if self.modality['use_camera']:
            ...
            for cam_type, cam_info in info['cams'].items():
                ...
                image_paths.append(strip_prefix(cam_info['data_path']))
        ...
        if self.modality['use_radar'] == True:
            input_dict.update(
                dict(
                    radar_sweeps = [strip_prefix(s) for s in info['radar_sweeps']],
