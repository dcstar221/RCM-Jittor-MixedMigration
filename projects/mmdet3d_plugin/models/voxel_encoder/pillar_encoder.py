...
try:
    ...
    raise ImportError("torch_scatter is required but not installed. Please run: pip install torch-scatter -f https://data.pyg.org/whl/torch-1.13.1+cu116.html")
...
class DynamicPillarVFESimple2D(nn.Module):
    ...
    def forward(self, points, **kwargs):
        ...
        points_coords = torch.floor(
            (points[:, [1, 2]] - self.point_cloud_range[[0, 1]].to(dev)) / self.voxel_size[[0, 1]].to(dev)).int()
        mask = ((points_coords >= 0) & (points_coords < self.grid_size[[0, 1]].to(dev))).all(dim=1)
        points = points[mask]
